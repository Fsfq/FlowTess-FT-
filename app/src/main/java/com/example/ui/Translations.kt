...
object Translations {
    ...
    fun getLocalizedCubeSkinDesc(cubeSkinId: String, lang: Language): String = when (cubeSkinId) {
        "neon" -> when (lang) {
            Language.RU -> "Объемные грани с классической фаской"
            Language.UA -> "Об'ємні грані з класичною фаскою"
            Language.KK -> "Классикалық жиектері бар көлемді блоктар"
            Language.DE -> "Klassische 3D-Kanten mit Facettenschliff"
            Language.ZH -> "经典斜边立体质感方块"
            else -> "Classic beveled edges with clean depth"
        }
        "glass" -> when (lang) {
            Language.RU -> "Мягкая матовая текстура с гладким контуром"
            Language.UA -> "М'яка матова текстура з гладким контуром"
            Language.KK -> "Тегіс контурлы жұмсақ күңгірт құрылым"
            Language.DE -> "Sanfte matte Textur mit klarer Kontur"
            Language.ZH -> "细腻柔和的磨砂材质"
            else -> "Soft matte texture with smooth outline"
        }
        "flat" -> when (lang) {
            Language.RU -> "Строгий плоский стиль с тонкой окантовкой"
            Language.UA -> "Строгий плоский стиль з тонкою окантовкою"
            Language.KK -> "Жіңішке жиекті қатаң тегіс стиль"
            Language.DE -> "Flaches Design mit feinem Innenrand"
            Language.ZH -> "利落简洁的细边平整质感"
            else -> "Crisp flat design with subtle inset border"
        }
        "material" -> when (lang) {
            Language.RU -> "Глянцевый стеклянный кристалл с диагональным бликом"
            Language.UA -> "Глянцевий скляний кристал з діагональним відблиском"
            Language.KK -> "Диагональды жарқылы бар шыны кристалды блок"
            Language.DE -> "Glänzender Glaskristall mit Lichtreflex"
            Language.ZH -> "带对角反射高光的晶透玻璃质感"
            else -> "Glossy crystal glass with diagonal specular sheen"
        }
        "glowing_jewel" -> when (lang) {
            Language.RU -> "Сегментированная сетка из четырех мини-блоков"
            Language.UA -> "Сегментована сітка з чотирьох міні-блоків"
            Language.KK -> "Төрт шағын блоктан тұратын тор"
            Language.DE -> "Segmentiertes Gitter aus vier Miniblöcken"
            Language.ZH -> "四宫格精细微块纹理"
            else -> "Segmented 2x2 grid pattern"
        }
        "steampunk" -> when (lang) {
            Language.RU -> "Шлифованная карбоновая плитка с микро-структурой"
            Language.UA -> "Шліфована карбонова плитка з мікро-структурою"
            Language.KK -> "Микроқұрылымды өңделген карбон плиткасы"
            Language.DE -> "Gebürstete Carbon-Kachel mit Mikrostruktur"
            Language.ZH -> "拉丝微纹理碳纤维板"
            else -> "Brushed carbon tile with fine horizontal micro-grooves"
        }
        "red_gradient" -> when (lang) {
            Language.RU -> "Вертикальный темно-красный градиент"
            Language.UA -> "Вертикальний темно-червоний градієнт"
            Language.KK -> "Тік қою қызыл градиент"
            Language.DE -> "Vertikaler karmesinroter Farbverlauf"
            Language.ZH -> "深红垂直渐变"
            else -> "Vertical crimson gradient"
        }
        "green_gradient" -> when (lang) {
            Language.RU -> "Вертикальный изумрудный градиент"
            Language.UA -> "Вертикальний смарагдовий градієнт"
            Language.KK -> "Тік зүмрәт градиент"
            Language.DE -> "Vertikaler smaragdgrüner Farbverlauf"
            Language.ZH -> "翠绿垂直渐变"
            else -> "Vertical emerald gradient"
        }
        "blue_gradient" -> when (lang) {
            Language.RU -> "Вертикальный кобальтовый градиент"
            Language.UA -> "Вертикальний кобальтовий градієнт"
            Language.KK -> "Тік кобальт градиент"
            Language.DE -> "Vertikaler kobaltblauer Farbverlauf"
            Language.ZH -> "钴蓝垂直渐变"
            else -> "Vertical cobalt gradient"
        }
        "purple_gradient" -> when (lang) {
            Language.RU -> "Вертикальный аметистовый градиент"
            Language.UA -> "Вертикальний аметистовий градієнт"
            Language.KK -> "Тік аметист градиент"
            Language.DE -> "Vertikaler violetter Farbverlauf"
            Language.ZH -> "紫晶垂直渐变"
            else -> "Vertical amethyst gradient"
        }
        else -> ""
    }
    ...
    fun getLocalizedAvatarFrameTitle(frameId: String, lang: Language): String = when (frameId.lowercase()) {
        ...
        "frame_orange" -> when (lang) {
            Language.RU -> "Янтарь"
            Language.UA -> "Бурштин"
            Language.KK -> "Кәріптас"
            Language.DE -> "Bernstein"
            Language.ZH -> "琥珀"
            else -> "Amber"
        }
        "frame_red" -> when (lang) {
        ...
        "chrono_gl", "gradient_frame" -> when (lang) {
            Language.RU -> "Радужный кристалл"
            Language.UA -> "Райдужний кристал"
            Language.KK -> "Кемпірқосақ кристал"
            Language.DE -> "Regenbogenkristall"
            Language.ZH -> "彩虹水晶"
            else -> "Celestial Iris"
        }
        // Legacy fallbacks
    ...
    fun getLocalizedAvatarFrameDesc(frameId: String, lang: Language): String = when (frameId.lowercase()) {
        ...
        "frame_white" -> when (lang) {
            Language.RU -> "Благородная платина с зеркальным металлическим блеском"
            Language.UA -> "Шляхетна платина з дзеркальним металевим блиском"
            Language.KK -> "Айнадай металл жылтыры бар асыл платина"
            Language.DE -> "Edles Platin mit spiegelndem metallischem Glanz"
            Language.ZH -> "镜面金属质感的尊贵铂金相框"
            else -> "Noble platinum with reflective metallic chrome finish"
        }
        "frame_blue" -> when (lang) {
            Language.RU -> "Глубокий сияющий кобальт полированного сапфира"
            Language.UA -> "Глибокий сяючий кобальт полірованого сапфіру"
            Language.KK -> "Жылтыратылған сапфирдің терең жарқыраған кобальты"
            Language.DE -> "Tiefer Kobaltschimmer von poliertem Saphir"
            Language.ZH -> "抛光蓝宝石深邃闪耀的钴蓝质感"
            else -> "Deep cobalt luster of polished sapphire"
        }
        "frame_green" -> when (lang) {
            Language.RU -> "Насыщенный минеральный блеск природного изумруда"
            Language.UA -> "Насичений мінеральний блиск природного смарагду"
            Language.KK -> "Табиғи зүбаржаттың қанық минералды жылтыры"
            Language.DE -> "Reicher Mineralglanz von natürlichem Smaragd"
            Language.ZH -> "天然祖母绿的浓郁矿物光泽"
            else -> "Rich mineral luster of imperial emerald"
        }
        "frame_yellow" -> when (lang) {
            Language.RU -> "Роскошный теплый перелив полированного золота"
            Language.UA -> "Розкішний теплий перелив полірованого золота"
            Language.KK -> "Жылтыратылған алтынның сәнді жылы жарқылы"
            Language.DE -> "Luxuriöser warmer Schimmer von poliertem Gold"
            Language.ZH -> "抛光黄金奢华温润的流光溢彩"
            else -> "Luxurious warm gradient of polished gold"
        }
        "frame_orange" -> when (lang) {
            Language.RU -> "Теплый отблеск брашированной меди и янтаря"
            Language.UA -> "Теплий відблиск брашованої міді та бурштину"
            Language.KK -> "Өңделген мыс пен кәріптастың жылы жарқылы"
            Language.DE -> "Warmer Schein von gebürstetem Kupfer und Bernstein"
            Language.ZH -> "拉丝铜与温润琥珀的温暖光晕"
            else -> "Warm brushed copper and radiant amber glow"
        }
        "frame_red" -> when (lang) {
            Language.RU -> "Огненные грани драгоценного рубина"
            Language.UA -> "Вогняні грані дорогоцінного рубіна"
            Language.KK -> "Бағалы жақұттың отты қырлары"
            Language.DE -> "Feurige Lichtbrechung von edlem Rubin"
            Language.ZH -> "瑰丽红宝石的深邃璀璨折射"
            else -> "Deep jewel refraction of vivid ruby"
        }
        "frame_purple" -> when (lang) {
            Language.RU -> "Королевский аметист с призматическим сиянием"
            Language.UA -> "Королівський аметист з призматичним сяйвом"
            Language.KK -> "Призмалық жарқылы бар корольдік аметист"
            Language.DE -> "Königlicher Amethyst mit prismatischem Glanz"
            Language.ZH -> "散发棱镜折射光芒的皇家紫水晶"
            else -> "Deep prism purple of royal amethyst"
        }
        "frame_dark" -> when (lang) {
            Language.RU -> "Матовый обсидиановый титан с темным отблеском"
            Language.UA -> "Матовий обсидіановий титан з темним відблиском"
            Language.KK -> "Қараңғы жылтыры бар күңгірт обсидиан титаны"
            Language.DE -> "Mattes Obsidian-Titan mit dunklem Schimmer"
            Language.ZH -> "沉稳内敛的哑光黑曜石钛合金质感"
            else -> "Dark matte obsidian titanium with sleek luster"
        }
        "chrono_gl", "gradient_frame", "neon_ae", "gold_ma", "omega_ti" -> when (lang) {
            Language.RU -> "Тонкий многотональный перелив кристальной призмы"
            Language.UA -> "Тонкий багатотональний перелив кристальної призми"
            Language.KK -> "Кристалды призманың нәзік көп реңкті жарқылы"
            Language.DE -> "Subtiler mehrfarbiger Regenbogenschimmer einer Himmels-Iris"
            Language.ZH -> "空灵微彩的多维幻彩水晶流光"
            else -> "Subtle multi-tonal rainbow luster of celestial iris"
        }
        else -> ""
    }
