  ...
  signingConfigs {
    create("release") {
      ...
      if (keyFile.exists()) {
        ...
        // Fallback to debug keystore for local building when release jks not yet configured
    ...
    create("debugConfig") {
      val defaultDebugKeystore = file("${System.getProperty("user.home")}/.android/debug.keystore")
      storeFile = if (file("${rootDir}/debug.keystore").exists()) file("${rootDir}/debug.keystore") else defaultDebugKeystore
      storePassword = "android"
      keyAlias = "androiddebugkey"
      keyPassword = "android"
    }
...
androidComponents {
  onVariants { variant ->
    ...
    val baseVersionCode = android.defaultConfig.versionCode ?: 19
    val baseVersionName = (android.defaultConfig.versionName ?: "0.96.3").replace(" ", "-")
    ...
    variant.outputs.forEach { output ->
      ...
      (output as? com.android.build.api.variant.impl.VariantOutputImpl)?.outputFileName?.set("FT-$baseVersionName-$abi.apk")
    }
  }
}
